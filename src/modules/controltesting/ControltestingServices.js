...
function getService(service) {
  switch (service) {
    ...
    case "getroduedate":
      ...
      break;
    case "getpriorissue":
      return "/prior-issue/fetchpriorissue";
      break;
  }
}
...
export function getServiceDatas(service, id, type) {
  let API_BASE_URL = getService(service);
  if (id) {
    API_BASE_URL = API_BASE_URL + "/" + id + "?frameWork=" + type;
  }
  return axios.get(API_BASE_URL);
}
...
export function getPriorIssues(service, objectid, program, processid) {
  const API_BASE_URL = getService(service);
  const param =
    "?" +
    "objectId=" +
