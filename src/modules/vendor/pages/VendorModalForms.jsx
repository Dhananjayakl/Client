...
const VendorModalForm = ({
  ...
  }, [QSTFields, formMethods.watch()]);

  const Ratings = formMetaData.configurationFormMetaData.RTG;
  function handleSubmit(questions, options) {
    setIsSubmitted(true);
    const scores = calculateScore(questions, options, formMethods);
    const totalScore = scores.reduce((sum, score) => sum + score, 0);
    const rating = getRatingAndScoreByTotalScore(totalScore, Ratings);
    console.log(rating, "rating we got here");
    console.log("Scores:", scores);
    console.log("Total Score:", totalScore);
    const isValid = validateResponses(questions, formMethods);
    if (isValid) {
      formMethods.setValue("preliminaryScore", totalScore);
      formMethods.setValue("preliminaryRating", rating);
    ...
    try {
      ...
      response.data.QST.forEach((question) => {
        QSTappend({
          ...
          rating: question.qst_rating_scale,
            ...
            <>
              {PGEPages.map((page) => (
                <Card key={page.pge_id}>
                  <Card.Header>
                    <Card.Title>{page.pge_title}</Card.Title>
                  </Card.Header>
                </Card>
              ))}

              {SECFields.map((section) => (
                  ...
                  <Section title={section.secTitle}>
                    {QSTFields.filter(
                      ...
                      const questionRowIndex = QSTFields?.findIndex(
                        (ques) => ques?.questionCode === question?.questionCode
                      );
                      ...
                      switch (question.qstType) {
                        ...
                        case "boolean":
                          qtype =
                            question?.qstBooleanType === 1 ? "check" : "switch";
                          break;
                        case "date":
                          qtype = "date";
                      ...
                      return (
                        ...
                        >
                          ...
                          <Col md={8}>{question.questionText}</Col>
                          <Col md={4}>
                            <FormControl
                              control={control}
          ...
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div>
              <Button
                className="me-2"
                onClick={() => {
                  handleSubmit(QSTFields, OPTFields);
                }}
              >
                Submit
              </Button>
