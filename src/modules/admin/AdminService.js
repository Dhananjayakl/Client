...
export function getBusinessUnit(service, BId) {
  const API_BASE_URL = getService(service) + BId;
...
export function getCacheData(service) {
  let API_BASE_URL = getService(service);
  return axios.get(API_BASE_URL);
}

export function deleteChache(service, cachename) {
  let API_BASE_URL = getService(service);
  if (cachename) {
    API_BASE_URL = API_BASE_URL + "/" + cachename;
  }
...
export function deleteFromRelation(service, objectid) {
  const API_BASE_URL = getService(service);
  return axios.delete(API_BASE_URL + "/" + objectid);
}
