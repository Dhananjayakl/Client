...
const JSHook = (form, formMetaData, formMethods, formValues) => {
  form.onLoad = (props) => {
    ...
    formMetaData.fields.specificDate.visible = false;
    const hierarchyValue = formMethods.getValues("hierarchy");

    const execFreqncy = formMethods.getValues("execFreqncy");
    ...
    if (execFreqncy && execFreqncy == 6) {
      formMetaData.fields.specificDate.visible = true;
      formMetaData.fields.specificDate.required = true;
    } else {
      formMetaData.fields.specificDate.visible = false;
      formMetaData.fields.specificDate.required = false;
    }
  ...
  form.execFreqncy.onChange((value) => {
    formValues.execFreqncy = value;
