...
import { getroduedates } from "../RiskServices";
...
const JSHook = (
  ...
  formMetaData.fields.dueDate.editable = false;
  //Field Title based on framework type
  useEffect(() => {
    if (formValues.assessmentType == null) {
      formMethods.setValue("assessmentType", 2);
    }

    const initialFrameworkValue = formMethods.getValues("framework");
    updateFieldTitle(initialFrameworkValue);

    form.framework.onChange((value) => {
      updateFieldTitle(value);
    });
  }, []);

  const updateFieldTitle = (value) => {
    if (value === 1) {
      setFieldTitle(t("Requirement(s)"));
    } else if (value === 2) {
      setFieldTitle(t("Risk(s)"));
    } else {
      setFieldTitle("");
    }
  };
